...
## 2. ТОП-8 граблей из практики (читать первыми)
...
### Грабля 8. Хардкод API, Telegram и CRM внутри сайта

**Запрещено:**

- хардкодить URL обработчика заявок прямо в компоненте формы;
- хранить `TELEGRAM_BOT_TOKEN`, CRM-token, webhook CRM или другие серверные секреты в Astro-сайте;
- делать отдельную серверную интеграцию Telegram/CRM внутри каждого клиентского сайта без явного решения по проекту;
- привязывать сайт к AMS-инфраструктуре так, чтобы при передаче клиенту пришлось переписывать форму.

**Правильно:** сложные формы, квизы и заявки отправляются во внешний API-сервис заявок, по умолчанию `AMS Leads API`. Сайт знает только публичный endpoint и идентификатор проекта через переменные окружения:
...
Серверные секреты Telegram, CRM, рекламных кабинетов и webhooks хранятся вне сайта: в Doppler / server env / инфраструктуре API. `.env.example` остается пустым шаблоном.

Если клиент забирает сайт из сопровождения, меняется `PUBLIC_LEADS_API_URL` и/или `PUBLIC_PROJECT_ID`, а не код формы.
...
### 9.6. Формы и внешний API заявок

Если форма просто ведет на телефон, Telegram-ссылку или почту, внешний API не нужен.

Если форма отправляет заявку, квиз, обратный звонок, лид-магнит, рекламное событие, данные в Telegram, CRM, таблицу или любую другую интеграцию, она считается сложной формой и подключается через внешний API-сервис.

**Дефолт для проектов АМС:** `AMS Leads API`.

Фронтенд Astro не должен знать Telegram-token, CRM-token, webhook CRM или внутренние ключи рекламных систем. Он отправляет нормализованную заявку:

```ts
const leadsApiUrl = import.meta.env.PUBLIC_LEADS_API_URL;
const projectId = import.meta.env.PUBLIC_PROJECT_ID;

await fetch(leadsApiUrl, {
  method: 'POST',
...
Обязательные env-переменные для сайта:
...
Допустимые публичные переменные:
...
Недопустимые публичные переменные:
...
Если проект деплоится как static site на Nginx, нельзя рассчитывать на `src/pages/api/*.ts` внутри Astro для обработки заявок. API должен быть внешним сервисом или отдельным backend-процессом за Nginx proxy.

Если по конкретному проекту принято решение делать backend внутри Astro, это фиксируется отдельно в `SITE_ARCHITECTURE.md` / deploy-документе, а проект переводится на server output с adapter. Это исключение, не дефолт.
...
| 5 | Static output на Nginx не поддерживает runtime API routes | Формы отправлять во внешний API-сервис (`PUBLIC_LEADS_API_URL`) или переводить проект на server output с adapter |
...
| 8 | API route в `src/pages/api` не сработает после static deploy | Не использовать как обработчик заявок для Nginx static; нужен AMS Leads API или отдельный backend |
...
### 15.5. Проверка форм и API-переменных

```bash
grep -rn "api/contact\|TELEGRAM_BOT_TOKEN\|TELEGRAM_CHAT_ID\|CRM_TOKEN\|WEBHOOK" src/
grep -rn "fetch(.*http\|fetch(.*https" src/
grep -rn "PUBLIC_TELEGRAM\|PUBLIC_CRM\|PUBLIC_.*WEBHOOK" src/ .env.example
```

Если сайт использует сложные формы, проверить:

- есть `PUBLIC_LEADS_API_URL=` в `.env.example`;
- есть `PUBLIC_PROJECT_ID=` в `.env.example`;
- endpoint формы берется из `import.meta.env.PUBLIC_LEADS_API_URL`;
- идентификатор проекта берется из `import.meta.env.PUBLIC_PROJECT_ID`;
- Telegram/CRM/webhook-секреты отсутствуют в коде сайта и `.env.example`;
- при static deploy нет зависимости от `src/pages/api/*.ts` как runtime-обработчика заявок.

### 15.6. astro check + build
...
- Добавлено правило для сложных форм: заявки, квизы, Telegram, CRM, рекламные события и будущие интеграции идут через внешний API-сервис, по умолчанию `AMS Leads API`
- Зафиксированы обязательные публичные переменные `PUBLIC_LEADS_API_URL` и `PUBLIC_PROJECT_ID`
- Запрещен хардкод Telegram/CRM/webhook-секретов внутри Astro-сайтов
- Уточнены грабли static deploy: `src/pages/api/*.ts` не является runtime API после деплоя статического сайта на Nginx
- Pre-deploy scan расширен проверкой форм и API-переменных
